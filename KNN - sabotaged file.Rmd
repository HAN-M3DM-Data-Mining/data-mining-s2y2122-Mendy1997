...
propDiag <- round(prop.table * 100 , digits = 1)
...
```{r}
summary(cleanDF[c("radius_mean, "smoothness_mean")])
```
...
```{r}
cleanDF_test_pred <- knn(train = as.matrix(trainDF_feat), test = as.matrix(testDF_feat) = as.matrix(trainDF_labels), k = 21)
